    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('pos.edit_user') }}</h1>
                    <p class="text-sm text-gray-600 mt-1">{{ __('pos.update_user_details') }}</p>
                </div>
            ...
            <form action="{{ route('owner.users.update', $user) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('pos.password') }} <span class="text-gray-500">({{ __('pos.leave_blank_keep_current') }})</span>
                    </label>
                    <input 
                        type="password" 
                        name="password" 
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-start p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                            <input 
                                ...
                                {{ old('role', $currentRole) === 'manager' ? 'checked' : '' }}
                                required
                            >
                            <div>
                        ...
                        <label class="flex items-start p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                            <input 
                                ...
                                value="salesman" 
                                class="mt-1 mr-3 text-blue-600 focus:ring-blue-500"
                                {{ old('role', $currentRole) === 'salesman' ? 'checked' : '' }}
                            >
                            <div>
                        ...
                        <label class="flex items-start p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 {{ !$posEnabled ? 'opacity-50 cursor-not-allowed' : '' }}">
                            <input 
                                ...
                                value="cashier" 
                                class="mt-1 mr-3 text-blue-600 focus:ring-blue-500"
                                {{ old('role', $currentRole) === 'cashier' ? 'checked' : '' }}
                                {{ !$posEnabled ? 'disabled' : '' }}
                            >
                            <div>
